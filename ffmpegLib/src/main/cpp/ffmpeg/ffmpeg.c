...
static void do_video_out(OutputFile *of,
    ...
    if (ist && ist->st->start_time != AV_NOPTS_VALUE && ist->first_dts != AV_NOPTS_VALUE &&
        ost->frame_rate.num)
...
static int get_input_packet(InputFile *f, AVPacket **pkt) {
    if (f->readrate || f->rate_emu) {
        ...
        for (i = 0; i < f->nb_streams; i++) {
            ...
            stream_ts_offset = FFMAX(ist->first_dts != AV_NOPTS_VALUE ? ist->first_dts : 0,
                                     file_start);
...
int ffmpeg_run(int argc, char **argv) {
    ...
    if (setjmp(jump_buf)) {
        main_return_code = 1;
        LOGI("main_return_code=%d", main_return_code);
        goto end;
    }
