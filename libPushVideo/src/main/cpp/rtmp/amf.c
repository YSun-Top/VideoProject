...
AMFProp_Encode(AMFObjectProperty *prop, char *pBuffer, char *pBufEnd) {
    ...
    switch (prop->p_type) {
        case AMF_NUMBER:pBuffer = AMF_EncodeNumber(pBuffer, pBufEnd, prop->p_vu.p_number);
            break;

        case AMF_BOOLEAN:pBuffer = AMF_EncodeBoolean(pBuffer, pBufEnd, prop->p_vu.p_number != 0);
            break;

        case AMF_STRING:pBuffer = AMF_EncodeString(pBuffer, pBufEnd, &prop->p_vu.p_aval);
            break;
        ...
        case AMF_OBJECT:pBuffer = AMF_Encode(&prop->p_vu.p_object, pBuffer, pBufEnd);
            break;

        default:RTMP_Log(RTMP_LOGERROR, "%s, invalid type. %d", __FUNCTION__, prop->p_type);
            pBuffer = NULL;
    };
...
AMFProp_Decode(AMFObjectProperty *prop, const char *pBuffer, int nSize,
    ...
    switch (prop->p_type) {
        ...
        case AMF_NULL:
        case AMF_UNDEFINED:
        case AMF_UNSUPPORTED:prop->p_type = AMF_NULL;
            break;
        case AMF_REFERENCE: {
...
AMF3Prop_Decode(AMFObjectProperty *prop, const char *pBuffer, int nSize,
    ...
    switch (type) {
        case AMF3_UNDEFINED:
        case AMF3_NULL:prop->p_type = AMF_NULL;
            break;
        case AMF3_FALSE:prop->p_type = AMF_BOOLEAN;
            prop->p_vu.p_number = 0.0;
            break;
        case AMF3_TRUE:prop->p_type = AMF_BOOLEAN;
...
AMFProp_Dump(AMFObjectProperty *prop) {
    ...
    switch (prop->p_type) {
        case AMF_NUMBER:snprintf(str, 255, "NUMBER:\t%.2f", prop->p_vu.p_number);
            break;
        case AMF_BOOLEAN:
        ...
        case AMF_DATE:
            ...
            break;
        default:snprintf(str, 255, "INVALID TYPE 0x%02x", (unsigned char) prop->p_type);
    }
